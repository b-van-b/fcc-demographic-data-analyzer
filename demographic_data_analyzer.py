...
def calculate_demographic_data(print_data=True):
    ...
    advanced_ed_mask = ((df.education == "Bachelors") | (df.education == "Masters") | (df.education == "Doctorate"))
    ...
    higher_education_rich = round(len(df[advanced_ed_mask & rich_mask]) / higher_education * 100,1)
    lower_education_rich = round(len(df[lower_ed_mask & rich_mask]) / lower_education * 100,1)
    ...
    richest_countries = df[df.salary == ">50K"].groupby('native-country').count() / df.groupby('native-country').count()
    ...
    highest_earning_country_percentage = round(richest_countries.iloc[0].salary * 100,1)
